...
jobController.delete = async (req, res, next) => {
  ...
  try {
    ...
    if ( !job.username._id.equals(req.user.payload._id)) {
      sendMail(
        job.username.email,
...
jobController.searchbytitle = async (req, res, next) => {

  try {
    const result = await Job.find({title :  {'$regex': req.params.title,$options:'$i'} });
    return res.send({ result });
  } catch (error) {
    next(error);
  }
